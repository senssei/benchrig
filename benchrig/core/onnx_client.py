...
def default_models_dir() -> str:
    """Model directory: $BENCHRIG_MODEL_DIRS (first entry), then ./models if it exists, then ~/.benchrig/models."""
    env = os.environ.get("BENCHRIG_MODEL_DIRS", "").split(os.pathsep)[0]
    if env:
        return os.path.abspath(os.path.expanduser(env))
    if os.path.isdir("models"):
        return os.path.abspath("models")
    return os.path.expanduser(os.path.join("~", ".benchrig", "models"))
...
class OnnxGenAiClient(BaseRuntimeClient):
    ...
    def __init__(
        ...
        self.models_dir = os.path.abspath(models_dir) if models_dir else default_models_dir()
