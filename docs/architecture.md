...
Located in [`benchrig/core/client.py`](https://github.com/senssei/benchrig/blob/main/benchrig/core/client.py) and [`benchrig/core/onnx_client.py`](https://github.com/senssei/benchrig/blob/main/benchrig/core/onnx_client.py), this base class decouples test execution from the underlying inference engine:
...
* **`PrismClient`**: A `FoundryClient` subclass for a [Prism](https://github.com/senssei/prism-local) server (`prism serve`). The endpoint is always explicit (`prism.base_url`, default `http://127.0.0.1:5272/v1`) and the `foundry` CLI is never used; an optional bearer token comes from `PRISM_API_KEY`. Because one Prism endpoint fronts several engines, the engine of each model is taken from the server's `owned_by` field (or the `ollama:` prefix) and copied into every result record together with the `device` Prism reports. See [Runtimes](runtimes.md#prism).
...
Located in [`benchrig/core/hardware.py`](https://github.com/senssei/benchrig/blob/main/benchrig/core/hardware.py), this layer measures real-time physical resource saturation during model evaluation without requiring root or elevated privileges:
...
2. **Configuration File**: [`config.yaml`](https://github.com/senssei/benchrig/blob/main/benchrig/data/config.yaml) in the workspace root specifying endpoints, default models, timeouts, and thresholds.
3. **CLI Arguments**: Runtime flags passed to [`benchrig`](https://github.com/senssei/benchrig/blob/main/benchrig/cli.py) override file configuration dynamically.
